...
public class Runner extends Thread {
    ...
    public synchronized void setReverse(boolean reverse) {this.reverse = reverse;}
    public void setRunnerF(Runner runnerF,Runner runnerS){
        this.runnerF = runnerF;
        this.runnerS = runnerS;
    }
    @Override
    public synchronized void run(){ rice();}

    private synchronized void rice() {
        ...
        if (!reverse){
            if (getName().equals("runner5")){
                ...
                System.out.println("бежит к " + runnerS.getName());
                try {
                    sleep(5000);
                } catch (InterruptedException ignored) {
            }
